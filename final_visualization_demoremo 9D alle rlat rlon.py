...
def main():
    ...
    # Nur Deutschland-Gitterpunkte auswählen
    coord_df_DE = coord_df.query(
        "lat >= 47.3 and lat <= 55.5 and lon >= 5.5 and lon <= 16.0"
    )
    ...
    # Sample für alles (Polygone + Parameter)
    print("\n Note: Using sample of grid points for visualization")
    # Bisher (nur jeder 2. Punkt)
    # coord_sample = coord_df_DE.iloc[::2].reset_index(drop=True)

    # Neu: alle verfügbaren Deutschland-Punkte
    coord_sample = coord_df_DE.reset_index(drop=True)

    gdf = create_grid_polygons(coord_sample, cell_size=.44)
