...
def load_netcdf_files():
    '''Load merged NetCDF files from CDS download directory'''
    print('Loading merged NetCDF files from CDS...')
    
    import glob
    
    # Find all merged NetCDF files
    ...
    print(f'✓ Found {len(netcdf_files)} merged NetCDF file(s)')
    
    # Load first file to check structure
    try:
        ds_merged = xr.open_dataset(netcdf_files[0], engine='netcdf4')
        
        print(f'✓ Loaded example file: {os.path.basename(netcdf_files[0])}')
        print(f'  - Dimensions: {dict(ds_merged.dims)}')
        print(f'  - Variables: {list(ds_merged.data_vars.keys())}')
        print(f'  - Time steps: {len(ds_merged.valid_time)}')
        print(f'  - Grid: {len(ds_merged.latitude)} x {len(ds_merged.longitude)}')
        
        # Check which variables are present
        required_vars = ['swh', 't2m', 'tp', 'sst', 'u10', 'v10']
        missing_vars = [v for v in required_vars if v not in ds_merged.data_vars]
        ...
        # Return the list of files (we'll process them later)
        return netcdf_files, None, None
...
def main():
    print('=' * 70)
    print('CDS ERA5 Merged Data - GeoJSON Generator')
    print('Version 2.0: Processing merged NetCDF files')
    print('=' * 70)
    
    # Load NetCDF files (returns list of file paths)
    netcdf_files, _, _ = load_netcdf_files()
    if netcdf_files is None:
        print('\n✗ Failed to load NetCDF files')
        return
    
    print(f'\nProcessing {len(netcdf_files)} file(s)...')
    
    # Open all files as a single dataset using xarray's open_mfdataset
    # This automatically concatenates along the time dimension
    print('\nMerging all files into single dataset...')
    try:
        ...
        print(f'  - Total time steps: {len(ds_merged.valid_time)}')
        print(f'  - Date range: {pd.Timestamp(ds_merged.valid_time.values[0]).strftime("%Y-%m-%d")} to {pd.Timestamp(ds_merged.valid_time.values[-1]).strftime("%Y-%m-%d")}')
        print(f'  - Grid: {len(ds_merged.latitude)} x {len(ds_merged.longitude)}')
    ...
    # - ds_wave = merged dataset (has swh)
    # - ds_weather_inst = merged dataset (has t2m, sst, u10, v10)
    # - ds_weather_accum = merged dataset (has tp)
    # They're all the same now!
    
    ds_wave = ds_merged
    ds_weather_inst = ds_merged
    ds_weather_accum = ds_merged
    
    # Get target grid
    target_lat = ds_weather_inst['latitude'].values
    target_lon = ds_weather_inst['longitude'].values
    
    # Since wave data is already on the same grid, we don't need interpolation
    # But we'll create a compatible structure for the old code
    print('\nPreparing wave data (no interpolation needed - already on same grid)...')
    interpolated_wave = {
        'swh': ds_wave['swh'].values
    }
    print('✓ Wave data ready')
    ...
    print('Next step: Check geojson_daily/ directory')
    print('=' * 70)
